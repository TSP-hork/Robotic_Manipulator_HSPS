...
> **Status: In development.** The FOC algorithm has not been implemented yet.
> This section describes the target architecture.

The MCU firmware will run on the STM32H7A3 and will implement:

- **FOC (Field-Oriented Control):** Clarke/Park transforms, PI current
  regulators in the d-q reference frame.
...
- **Inverse kinematics solver:** Converts Cartesian commands from the
  Technologist Module into joint-space targets.
...
| Positioning accuracy (target) | < 0.5 mm              | < 0.1 mm             |
| Payload                       | TBD                   | TBD                  |
| Reach                         | TBD                   | TBD                  |
...
- [x] FPGA core (PWM generator, dead-time, encoder decoder, CSR)
...
- [ ] FOC algorithm implementation on STM32H7
- [ ] Motor spinning (full chain: ADC → FPGA → MCU → FPGA → motor)
- [ ] Remaining mechanical assembly and print iterations
